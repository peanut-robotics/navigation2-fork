  ...
  try {
    ...
    while (rclcpp::ok()) {
      ...
      if (!loop_rate.sleep()) {
        ...
        double expected_time = 1.0 / controller_frequency_;
        double miss_ratio = elapsed_seconds.count() / expected_time;
        RCLCPP_WARN(
          get_logger(), "Control loop missed its desired rate of %.3fHz ratio: (%.2f)",
          controller_frequency_, miss_ratio);
      }
    }
